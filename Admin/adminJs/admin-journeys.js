...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderTable(carts) {
        ...
        carts.forEach(cart => {
            ...
            const stopNames = (cart.stops || [])
    .filter(s => s)
    .map(s => s.name)
    .join(", ") || "No stops yet";
            const budget = cart.budget ? `$${cart.budget.totalBudget}` : "—";
            ...
            row.innerHTML = `
                ...
                <td>${budget}</td>
                <td>${date}</td>
                <td><span class="active">Saved</span></td>
                <td>
    ...
    function attachRowEvents() {
        ...
        document.querySelectorAll(".journey-table .view").forEach(button => {
            ...
            button.onclick = function () {
                ...
                const stopNames = (cart.stops || []).map(s => s.name).join(", ") || "No stops yet";

                document.getElementById("journeyUser").innerText = cart.ownerName || "Unknown";
                document.getElementById("journeyEmail").innerText = cart.ownerEmail || "—";
                ...
                document.getElementById("journeyBudget").innerText =
                    cart.budget ? `$${cart.budget.totalBudget} (spent $${cart.budget.spentSoFar})` : "Not set";
